...
with customer as (
    ...
    select * from {{ ref('parent_snowflake_project', 'stg_tpch_customers') }}

),
nation as (

    select * from {{ ref('parent_snowflake_project', 'stg_tpch_nations') }}
),
region as (

    select * from {{ ref('parent_snowflake_project', 'stg_tpch_regions') }}
