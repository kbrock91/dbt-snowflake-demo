...
{% macro store_test_results(results) %}
  ...
  {%- for result in results if result.node.resource_type == 'test' and result.status != 'skipped' and result.node.sources != [] -%}
  ...
  {%- for result in test_results %}
    ...
    select
      case 
        when '{{ test_name }}'::text = 'not_null' then '{{ column_name|escape }} is not null' 
        when '{{ test_name }}'::text = 'unique' then '{{ column_name|escape }} not in (select UNIQUE_FIELD from {{result.node.relation_name }})'  
        when '{{ test_name }}'::text = 'accepted_values' then '{{ column_name|escape }} not in (select VALUE_FIELD from {{result.node.relation_name }})'
